...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  private updateThrusterVisual(acceleration: { x: number; y: number }): void {
    ...
    (
      this.thrusterHaloEmitter.ops.angle as { loadConfig(cfg: object): void }
    ).loadConfig({ angle: { min: deg - 38, max: deg + 38 } });
    this.thrusterHaloEmitter.emitting = true;

    this.thrusterEmitter.setPosition(px, py);
    (
      this.thrusterEmitter.ops.angle as { loadConfig(cfg: object): void }
    ).loadConfig({ angle: { min: deg - 26, max: deg + 26 } });
  ...
  private updateThrusterSound(acceleration: { x: number; y: number }): void {
    const thrusting =
      acceleration.x * acceleration.x + acceleration.y * acceleration.y >= 1;
